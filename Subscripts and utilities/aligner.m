function [singlez_mat,shift_mat,ave_frame,err_mat,allreps_mat] = ...
    aligner(tar_path,tar_files,stim_num,rep_num,...
...
% allocate memory for the single reps also
allreps_mat = zeros(im_height,im_width,rep_num,time_num*(stim_num+exc_num*rep_num),'single');
...
%     histogram(pre_all(:),1000)
    ...
    if any(skip_stim==stim)
        ...
        % or not
        allreps_mat(:,:,:,st_counter:stim*time_num) = dfof_mat;
    end

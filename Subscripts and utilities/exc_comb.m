function [conc_trace,exc_trace,stim_num2,bout_signal,col_out,bta_out,corr_out,all_reps_out] = ...
    exc_comb(all_trace,exclude_vec,comb_vec,bout_stim,col_ave,bta_behav,corr_behav,all_trace_reps)
...
if ~isempty(comb_vec)
    ...
    %and the corr coeffs
    corr_out = comb_corr;
    % NOT IMPLEMENTED FOR REPS CAUSE NOT NEEDED WITH THESE DATA
    all_reps_out = [];
else
    ...
    bta_out = bta_behav;
    corr_out = corr_behav;
    all_reps_out = reshape(all_trace_reps,size(all_trace_reps,1),size(all_trace_reps,2)*size(all_trace_reps,3)*size(all_trace_reps,4));
end